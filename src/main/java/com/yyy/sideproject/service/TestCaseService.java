...
public class TestCaseService {
	...
	@Transactional
	public void updateTestCaseStatus(Long tcId, String status) {
		// 1. DB에서 해당 테스트 케이스를 찾아옴
		TestCase tc = testCaseRepository.findById(tcId)
				.orElseThrow(() -> new IllegalArgumentException("존재하지 않는 테스트 케이스입니다."));

		// 2. 상태 값을 새로운 값(SUCCESS 또는 FAIL)으로 변경
		tc.setStatus(status);

		// @Transactional이 붙어있어서 save()를 따로 안 해도 알아서 DB에 저장(Update)됩니다.
	}
}